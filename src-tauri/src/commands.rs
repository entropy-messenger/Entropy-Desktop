...
#[tauri::command]
pub fn protocol_establish_session(
...
#[tauri::command]
pub fn protocol_init(state: State<'_, DbState>) -> Result<serde_json::Value, String> {
    let conn_lock = state.conn.lock().unwrap();
    if let Some(conn) = conn_lock.as_ref() {
...
#[tauri::command]
pub fn protocol_replenish_pre_keys(state: State<'_, DbState>, count: u32) -> Result<serde_json::Value, String> {
    let lock = state.conn.lock().unwrap();
    if let Some(conn) = lock.as_ref() {
...
#[tauri::command]
pub fn protocol_verify_session(state: State<'_, DbState>, remote_hash: String, is_verified: bool) -> Result<(), String> {
    let lock = state.conn.lock().unwrap();
    if let Some(conn) = lock.as_ref() {
...
#[tauri::command]
pub fn protocol_create_group_distribution(state: State<'_, DbState>, group_id: String) -> Result<serde_json::Value, String> {
    let lock = state.conn.lock().unwrap();
    if let Some(conn) = lock.as_ref() {
...
#[tauri::command]
pub fn protocol_group_init(state: State<'_, DbState>, group_id: String) -> Result<serde_json::Value, String> {
    let lock = state.conn.lock().unwrap();
    if let Some(conn) = lock.as_ref() {
...
#[tauri::command]
pub fn protocol_group_encrypt(state: State<'_, DbState>, group_id: String, plaintext: String) -> Result<serde_json::Value, String> {
    let lock = state.conn.lock().unwrap();
    if let Some(conn) = lock.as_ref() {
...
#[tauri::command]
pub fn protocol_group_decrypt(state: State<'_, DbState>, group_id: String, sender_hash: String, msg_obj: serde_json::Value) -> Result<String, String> {
    let lock = state.conn.lock().unwrap();
    if let Some(conn) = lock.as_ref() {
        let mut gs = protocol::GroupState::load_from_db(conn, &group_id)?.ok_or("Group not found")?;
...
#[tauri::command]
pub fn protocol_process_group_distribution(state: State<'_, DbState>, sender_hash: String, dist_obj: serde_json::Value) -> Result<(), String> {
    let lock = state.conn.lock().unwrap();
    if let Some(conn) = lock.as_ref() {
...
#[tauri::command]
pub fn send_to_network(state: State<'_, NetworkState>, msg: String, is_binary: bool) -> Result<(), String> {
...
#[tauri::command]
pub fn dump_vault(state: State<'_, DbState>) -> Result<std::collections::HashMap<String, String>, String> {
    let lock = state.conn.lock().unwrap();
    if let Some(conn) = lock.as_ref() {
...
#[tauri::command]
pub fn restore_vault(state: State<'_, DbState>, data: std::collections::HashMap<String, String>) -> Result<(), String> {
    let lock = state.conn.lock().unwrap();
    if let Some(conn) = lock.as_ref() {
        for (k, v) in data {
            conn.execute(
