...
impl IdentityKeyStore for InMemorySignalStore {
    ...
    async fn save_identity(&mut self, address: &ProtocolAddress, identity: &IdentityKey) -> Result<IdentityChange, SignalProtocolError> {
    ...
    async fn is_trusted_identity(&self, address: &ProtocolAddress, identity: &IdentityKey, _direction: Direction) -> Result<bool, SignalProtocolError> {
...
impl PreKeyStore for InMemorySignalStore {
    async fn get_pre_key(&self, pre_key_id: PreKeyId) -> Result<PreKeyRecord, SignalProtocolError> {
        ...
        let bytes = lock.get(&pre_key_id).ok_or(SignalProtocolError::InvalidPreKeyId)?;
        PreKeyRecord::deserialize(bytes)
    }
    async fn save_pre_key(&mut self, pre_key_id: PreKeyId, record: &PreKeyRecord) -> Result<(), SignalProtocolError> {
...
impl SignedPreKeyStore for InMemorySignalStore {
    async fn get_signed_pre_key(&self, signed_pre_key_id: SignedPreKeyId) -> Result<SignedPreKeyRecord, SignalProtocolError> {
        let lock = self.signed_pre_keys.read().unwrap();
        let bytes = lock.get(&signed_pre_key_id).ok_or(SignalProtocolError::InvalidSignedPreKeyId)?;
        SignedPreKeyRecord::deserialize(bytes)
    }
    async fn save_signed_pre_key(&mut self, signed_pre_key_id: SignedPreKeyId, record: &SignedPreKeyRecord) -> Result<(), SignalProtocolError> {
...
impl KyberPreKeyStore for InMemorySignalStore {
    async fn get_kyber_pre_key(&self, kyber_prekey_id: KyberPreKeyId) -> Result<KyberPreKeyRecord, SignalProtocolError> {
        ...
        let bytes = lock.get(&kyber_prekey_id).ok_or(SignalProtocolError::InvalidKyberPreKeyId)?;
    ...
    async fn mark_kyber_pre_key_used(&mut self, _kyber_prekey_id: KyberPreKeyId, _ec_prekey_id: SignedPreKeyId, base_key: &PublicKey) -> Result<(), SignalProtocolError> {
        let mut lock = self.used_base_keys.write().unwrap();
        let bytes = base_key.serialize().to_vec();
        if lock.contains(&bytes) {
            return Err(SignalProtocolError::InvalidMessage(libsignal_protocol::CiphertextMessageType::PreKey, "reused base key".into()));
        }
        lock.insert(bytes);
        Ok(())
    }
...
async fn encryption_handshake() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 2. Bob generates and saves a PreKeyBundle
    ...
    bob_store.save_pre_key(bob_pre_key_id, &bob_pre_key_record).await?;
    ...
    let signature = bob_id_pair.private_key().calculate_signature(&bob_signed_pre_key_pair.public_key.serialize(), &mut rng)?;
    let bob_signed_pre_key_record = SignedPreKeyRecord::new(bob_signed_pre_key_id, timestamp, &bob_signed_pre_key_pair, &signature);
    bob_store.save_signed_pre_key(bob_signed_pre_key_id, &bob_signed_pre_key_record).await?;

    let bob_kyber_pre_key_id = KyberPreKeyId::from(1);
    let bob_kyber_pre_key_record = KyberPreKeyRecord::generate(kem::KeyType::Kyber1024, bob_kyber_pre_key_id, bob_id_pair.private_key())?;
    bob_store.save_kyber_pre_key(bob_kyber_pre_key_id, &bob_kyber_pre_key_record).await?;
    ...
    // 3. Alice processes Bob's bundle to establish a session
    ...
    // Alice should have sent a PreKeySignalMessage for the first message
    assert!(matches!(ciphertext, CiphertextMessage::PreKeySignalMessage(_)));
    ...
    // Bob's reply should be a standard SignalMessage (not PreKey) because the session is now established
    assert!(matches!(bob_ciphertext, CiphertextMessage::SignalMessage(_)));
...
async fn out_of_order_delivery() -> Result<(), Box<dyn std::error::Error>> {
    ...
    process_prekey_bundle(&bob_addr, &mut alice_store.clone(), &mut alice_store, &bob_bundle, std::time::SystemTime::now(), &mut rng).await?;
    ...
    let cipher1 = message_encrypt(msg1, &bob_addr, &alice_addr, &mut alice_store.clone(), &mut alice_store, std::time::SystemTime::now(), &mut rng).await?;
    let cipher2 = message_encrypt(msg2, &bob_addr, &alice_addr, &mut alice_store.clone(), &mut alice_store, std::time::SystemTime::now(), &mut rng).await?;
    let cipher3 = message_encrypt(msg3, &bob_addr, &alice_addr, &mut alice_store.clone(), &mut alice_store, std::time::SystemTime::now(), &mut rng).await?;

    // Bob receives them OUT OF ORDER: 1, then 3, then 2
    let dec1 = message_decrypt(&cipher1, &alice_addr, &bob_addr, &mut bob_store.clone(), &mut bob_store.clone(), &mut bob_store.clone(), &bob_store.clone(), &mut bob_store, &mut rng).await?;
    assert_eq!(dec1, msg1);

    let dec3 = message_decrypt(&cipher3, &alice_addr, &bob_addr, &mut bob_store.clone(), &mut bob_store.clone(), &mut bob_store.clone(), &bob_store.clone(), &mut bob_store, &mut rng).await?;
    assert_eq!(dec3, msg3);

    let dec2 = message_decrypt(&cipher2, &alice_addr, &bob_addr, &mut bob_store.clone(), &mut bob_store.clone(), &mut bob_store.clone(), &bob_store.clone(), &mut bob_store, &mut rng).await?;
    assert_eq!(dec2, msg2);
...
async fn malicious_bundle_rejection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Bob bundle is generated correctly
    let mut bob_bundle = generate_bundle(&mut bob_store, 2222, &mut rng).await?;

    // MALICIOUS ACTOR: Changes the identity key in the bundle but leaves the old signature
...
async fn extended_chat_stability() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..10 {
        ...
        let a_cipher = message_encrypt(a_msg.as_bytes(), &bob_addr, &alice_addr, &mut alice_store.clone(), &mut alice_store, std::time::SystemTime::now(), &mut rng).await?;
        let b_dec = message_decrypt(&a_cipher, &alice_addr, &bob_addr, &mut bob_store.clone(), &mut bob_store.clone(), &mut bob_store.clone(), &bob_store.clone(), &mut bob_store, &mut rng).await?;
        assert_eq!(b_dec, a_msg.as_bytes());

        // Bob -> Alice
        let b_msg = format!("Pong {}", i);
        let b_cipher = message_encrypt(b_msg.as_bytes(), &alice_addr, &bob_addr, &mut bob_store.clone(), &mut bob_store, std::time::SystemTime::now(), &mut rng).await?;
        let a_dec = message_decrypt(&b_cipher, &bob_addr, &alice_addr, &mut alice_store.clone(), &mut alice_store.clone(), &mut alice_store.clone(), &alice_store.clone(), &mut alice_store, &mut rng).await?;
...
async fn replay_attack_prevention() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let msg = b"Reusable?";
    let ciphertext = message_encrypt(msg, &bob_addr, &alice_addr, &mut alice_store.clone(), &mut alice_store, std::time::SystemTime::now(), &mut rng).await?;

    // First decryption works
    message_decrypt(&ciphertext, &alice_addr, &bob_addr, &mut bob_store.clone(), &mut bob_store.clone(), &mut bob_store.clone(), &bob_store.clone(), &mut bob_store, &mut rng).await?;

    // Second decryption with SAME ciphertext should fail because mark_kyber_pre_key_used will see the same base key
    let result = message_decrypt(&ciphertext, &alice_addr, &bob_addr, &mut bob_store.clone(), &mut bob_store.clone(), &mut bob_store.clone(), &bob_store.clone(), &mut bob_store, &mut rng).await;
...
async fn identity_rotation_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    process_prekey_bundle(&bob_addr, &mut alice_store.clone(), &mut alice_store, &bob_bundle1, std::time::SystemTime::now(), &mut rng).await?;

    // Alice has saved Bob's identity key now.
    assert!(alice_store.is_trusted_identity(&bob_addr, bob_bundle1.identity_key()?, Direction::Sending).await?);

    // Bob "reinstalls" and gets a new identity key
    ...
    // Alice tries to process the new bundle. 
    // is_trusted_identity should return false because the key has changed.
    let is_trusted = alice_store.is_trusted_identity(&bob_addr, bob_bundle2.identity_key()?, Direction::Sending).await?;
    assert!(!is_trusted);

    // When Alice saves the new identity, it should return IdentityChange::ReplacedExisting
    let change = alice_store.save_identity(&bob_addr, bob_bundle2.identity_key()?).await?;
...
async fn setup_test_user(name: &str, reg_id: u32, rng: &mut StdRng) -> Result<(InMemorySignalStore, ProtocolAddress), Box<dyn std::error::Error>> {
...
async fn generate_bundle(store: &mut InMemorySignalStore, reg_id: u32, rng: &mut StdRng) -> Result<PreKeyBundle, Box<dyn std::error::Error>> {
    ...
    let timestamp = Timestamp::from_epoch_millis(123456789);
    let signature = id_pair.private_key().calculate_signature(&signed_pre_key_pair.public_key.serialize(), rng)?;
    ...
    let kyber_pre_key_record = KyberPreKeyRecord::generate(kem::KeyType::Kyber1024, kyber_pre_key_id, id_pair.private_key())?;
